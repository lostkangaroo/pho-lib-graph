...
class FeedEdgeListTest extends TestCase {
  ...
  public function testFeedEdgeList() {
    ...
    $this->assertEquals($this->node1->edges()->toArray(), $node1->edges()
      ->toArray());
  }
  ...
  private function recreateNode(NodeInterface $node, /*?array*/ $list = NULL) {
    ...
    $node1_mock->shouldReceive("edges")
      ->andReturn(new EdgeList($node1_mock, (!is_null($list) ? $list : $node->edges()
        ->toArray())));
    return $node1_mock;
  }
  ...
  public function testSerializeThenAdd() {
    ...
    $this->assertEquals($node2_ins->current()->id(), $node1_outs->current()
      ->id());
    $this->assertEquals($node2_outs->current()->id(), $node1_ins->current()
      ->id());
    $this->assertEquals($this->node2->edges()
      ->from($node1->id())
      ->current()
      ->id(), $node1_outs->current()->id());
    $this->assertEquals($this->node2->edges()
      ->to($node1->id())
      ->current()
      ->id(), $node1_ins->current()->id());
    $this->assertCount(1, $this->node2->edges()->from($node1->id()));
    ...
    $this->assertContains((string) $node2_outs->current()
      ->id(), $node1_all_ids);
    $this->assertContains((string) $node2_alls->current()
      ->id(), $node1_all_ids);
    $node2_alls->next();
    $this->assertContains((string) $node2_alls->current()
      ->id(), $node1_all_ids);
  ...
  public function testWithNewEdgeType() {
    ...
    $this->assertCount(1, $this->node1->edges()
      ->to($this->node2->id(), $new_edge_class));
    $this->assertCount(1, $this->node2->edges()
      ->from($this->node1->id(), $new_edge_class));
    $this->assertCount(1, $this->node2->edges()
      ->between($this->node1->id(), $new_edge_class));
    $this->assertCount(2, $this->node1->edges()
      ->to($this->node2->id(), Edge::class));
    $this->assertCount(2, $this->node2->edges()
      ->from($this->node1->id(), Edge::class));
    $this->assertCount(2, $this->node2->edges()
      ->between($this->node1->id(), Edge::class));
  }
  ...
  public function testSerializeThenAddWithNewEdgeType() {
    ...
    $this->assertCount(1, $this->node2->edges()
      ->from($node1->id(), $new_edge_class));
    $this->assertCount(1, $this->node2->edges()
      ->between($node1->id(), $new_edge_class));
    $this->assertCount(2, $new_edge_list->to($this->node2->id(), Edge::class));
    $this->assertCount(2, $this->node2->edges()
      ->from($node1->id(), Edge::class));
    $this->assertCount(2, $this->node2->edges()
      ->between($node1->id(), Edge::class));
  }
  ...
  public function testSerializeThenAddThenSerializeThenAddAgain() {
    ...
    $node2_alls->rewind();
    //eval(\Psy\sh());
    $this->assertContains((string) $node2_ins->current()
      ->id(), array_map($get_id, $newest_edge_list->between($this->node2->id())
      ->getArrayCopy()));
    $this->assertContains((string) $this->node2->edges()
      ->to($this->node1->id())
      ->current()
      ->id(), $node1_all_ids);
  ...
  private function doSerializeThenAddThenSerializeThenAddAgainForToFromBetween($node1) {
    ...
    $this->assertCount(1, $newest_edge_list->to($this->node2->id()));
    $this->assertEquals($this->edge->id(), $newest_edge_list->to($this->node2->id())
      ->current()
      ->id());
    $this->assertCount(1, $newest_edge_list->to($this->node2->id()));
    ...
    $this->assertCount(3, array_map($get_id, $newest_edge_list->between($this->node2->id())
      ->getArrayCopy()));
